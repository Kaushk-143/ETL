...
export default function AssessmentStep({ data, students, onUpdate, schoolId }: AssessmentStepProps) {
  ...
  const handleBulkImport = async (importedData: any[]) => {
    try {
      ...
      // If this is raw file data, process it through the assessment processor
      if (importedData.length > 0 && typeof importedData[0] === 'object') {
        // Detect source and process
        const headers = Object.keys(importedData[0]);
        const source = assessmentProcessor.detectAssessmentSource(headers);
        
        // Process the assessment data
        const processedData = await assessmentProcessor.processAssessmentData(importedData, source);
      ...
      setError(`Failed to import assessments: ${err.message}`);
    } finally {
      setIsProcessing(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <BulkImport
          ...
          requiredFields={[
            'student_id', 'assessment_type', 'subject', 'grade_level',
            'test_date', 'scale_score', 'performance_level_text'
          ]}
          template={{
            student_id: students[0]?.student_id || 'student-uuid',
            assessment_type: 'NJSLA_ELA',
            subject: 'ELA',
            grade_level: '4',
            school_year: `${CURRENT_YEAR}-${CURRENT_YEAR+1}`,
            ...
            raw_score: 45,
            scale_score: 725,
            performance_level_text: 'Meeting',
            min_possible_score: '650',
            max_possible_score: '850',
            student_growth_percentile: 65
          }}
          description="Upload a CSV or Excel file containing assessment records. Student ID must match existing students from the Students step. The system supports LinkIt, NJSLA, and other assessment formats with automatic data mapping."
        />
      </div>

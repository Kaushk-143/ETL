...
import { Users, Plus, Trash2, Mail, Phone, GraduationCap, UserPlus, Heart, AlertCircle, BookOpen } from 'lucide-react';
...
export default function CombinedStudentsStep({ studentData, needsData, onStudentsUpdate, onNeedsUpdate }: CombinedStepProps) {
    ...
    try {
      ...
      importedData.forEach(row => {
        ...
        const student: Student = {
          ...
          has_iep: row.has_iep || row.iep === 'Y' || row.iep === 'Yes',
          is_economically_disadvantaged: row.is_economically_disadvantaged || row.economically_disadvantaged === 'Y' || row.economically_disadvantaged === 'Yes',
          individualized_notes: row.individualized_notes
        };
        ...
        if (row.has_iep || row.iep === 'Y' || row.iep === 'Yes') {
          const iepNeed: IndividualizedNeed = {
            ...
            details: {
              status: 'Active',
              spec_ed_status: row.spec_ed_status,
              spec_ed: row.spec_ed,
              has_active_iep: true,
              additional_notes: row.individualized_notes
            }
          };
  ...
  const getStudentNeeds = (studentId: string) => {
    return cachedNeedsData.filter(need => need.student_id === studentId);
  };

  const formatNeedsForDisplay = (needs: IndividualizedNeed[]) => {
    if (needs.length === 0) return 'No individualized needs';
